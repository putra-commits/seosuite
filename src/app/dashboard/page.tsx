...
const MODULES = [
  ...
  { name: 'Arsitektur Link', href: '/links', icon: Unlink, color: 'text-blue-500', bg: 'bg-blue-500/10' },
    ...
    <div className="p-8 lg:p-16 max-w-7xl mx-auto space-y-20 font-sans">
      ...
      <header className="relative">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-6xl font-black tracking-tighter mb-6 italic text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-8 space-y-12">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-black text-zinc-600 uppercase tracking-[0.3em] flex items-center gap-3 italic">
              ...
              >
                <div className="relative z-10">
                  <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="px-3 py-1 rounded-lg bg-zinc-950 border border-zinc-800 text-[10px] font-black uppercase tracking-widest text-zinc-600 group-hover:text-zinc-300 transition-colors italic">
                      {item.status}
                    </div>
                  </div>
                  <h3 className="text-xl font-black text-white mb-4 flex items-center gap-2 italic uppercase tracking-tighter">
                    {item.name}
                    <ExternalLink className="w-4 h-4 text-zinc-800 group-hover:text-blue-500 transition-all" />
                  </h3>
          ...
          <div className="rounded-[3rem] bg-zinc-900/10 border border-zinc-800 p-10 space-y-8">
             ...
             <div className="h-[200px] flex items-end gap-2 relative">
                ...
                <div className="flex-1 h-[35%] bg-blue-500/10 border-t border-blue-500/50 rounded-t" />
                <div className="flex-1 h-[50%] bg-blue-500/20 border-t border-blue-500/70 rounded-t" />
                <div className="flex-1 h-[75%] bg-emerald-500/30 border-t border-emerald-500 rounded-t shadow-[0_-10px_20px_rgba(16,185,129,0.1)] flex items-center justify-center">
                   <span className="text-[8px] font-bold text-emerald-400 -rotate-90 italic">BREAKOUT</span>
                </div>
             ...
             <div className="grid grid-cols-2 gap-8 pt-4 border-t border-zinc-900">
                <div>
                   ...
                   <p className="text-xl font-black text-white italic">+440.2%</p>
                </div>
                <div>
                   <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest mb-1">Status Asset</p>
                   <p className="text-xl font-black text-emerald-500 italic uppercase tracking-tighter">Sovereign Growth</p>
                </div>
             </div>
          </div>
          ...
          <div className="rounded-[3rem] bg-zinc-900/10 border border-zinc-800 p-10 space-y-8">
             <div className="flex justify-between items-center">
                <h3 className="text-lg font-black text-white italic uppercase tracking-tighter">
                   Performa <span className="text-blue-500">Pencarian</span>
                </h3>
          ...
          <div className="space-y-4">
            {MODULES.map((item) => (
              ...
              >
                ...
                <span className="text-sm font-black text-zinc-500 group-hover:text-zinc-100 transition-colors uppercase tracking-widest italic">{item.name}</span>
          ...
          <div className="p-10 rounded-[2.5rem] bg-zinc-900/50 border border-zinc-800 shadow-2xl relative overflow-hidden group">
            ...
            <div className="space-y-4">
              {[
                ...
                { label: 'PSI Automated Scan', status: 'Verified', color: 'bg-blue-500' },
                ...
                <div key={s.label} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                     ...
                     <span className="text-[10px] font-black text-zinc-100 uppercase italic">{s.status}</span>
                  </div>
                </div>
              ))}
            </div>
   ...
   return (
      <div className={`p-8 rounded-[2rem] bg-zinc-900/30 border ${color} hover:bg-zinc-900/60 transition-all cursor-default group`}>
         <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-[10px] font-black text-emerald-500 uppercase tracking-tighter italic">{sub}</span>
         </div>
         <p className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em] mb-1">{label}</p>
         <h4 className="text-4xl font-black text-white italic tracking-tighter italic">{value}</h4>
      </div>
   );
}
   ...
   return (
      <div className="flex items-center justify-between p-4 rounded-2xl bg-zinc-950/50 border border-zinc-900 hover:border-zinc-700 transition-all group cursor-default">
         <div className="flex items-center gap-4">
            ...
            <span className="text-sm font-bold text-zinc-300 group-hover:text-white transition-colors italic">{query}</span>
         </div>
         <div className="flex items-center gap-8">
            <div className="text-right">
               <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">Avg Pos</p>
               <p className="text-sm font-black text-white italic">{pos}</p>
            </div>
            <div className="text-right w-20">
               <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">Clicks</p>
               <p className="text-sm font-black text-emerald-500 italic">{clicks}</p>
            </div>
         </div>
      </div>
   );
